...
do
  mysql -u "$OPENSHIFT_DB_USERNAME" --password="$OPENSHIFT_DB_PASSWORD" --column-names=1 \
  -h "$OPENSHIFT_DB_HOST" mlstats -e "select current_timestamp() into @endperiod;
...
SELECT COALESCE(tdl.da,0) '$domain' FROM (select LEFT(interval_start, 10) fdl from time_intervals) list_all 
LEFT JOIN (select LEFT(first_date, 10) fdl, count(distinct(email_address)) da, SUBSTRING_INDEX(email_address, '@', -1) domain 
FROM messages m join messages_people p on m.message_ID = p.message_id where email_address LIKE '%$domain%' group by fdl order by first_date) tdl ON tdl.fdl = list_all.fdl" \
...
for domain in `sort $OPENSHIFT_REPO_DIR/libs/queries/working/top_five_domains | cut -d"," -f1 -`; do
